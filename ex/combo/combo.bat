...
ECHO Hold one of WASD
ECHO And one of IO
ECHO To make a Combo Move

PAUSE 

CALL :RADISH_PLAY 20 10 1 GAME
...
FOR /L %%# in () DO (
    TITLE Current Move : !comb!
    %RADISH_START_PARSE%
        IF %%C EQU 1 (
            IF not "!attack:%%B=!" == "%attack%" (
                SET "cur[attack]=%%B"
            ) else IF not "!move:%%B=!" == "%move%" (
                SET "cur[move]=%%B"
            )

            IF defined cur[attack] (
                IF defined cur[move] (
                    FOR /F "tokens=1-2" %%A in ("!cur[move]! !cur[attack]!") DO (
                        SET "comb=!move[%%A]! !attack[%%B]!"
                    )
                )
            )
        ) else (
            ...
            IF not defined cur[attack] (
                SET "comb="
            ) else IF not defined cur[move] (
                SET "comb="    
            )
        )
    %RADISH_END_PARSE%
