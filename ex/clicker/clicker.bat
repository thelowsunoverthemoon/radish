...
SET /A "num=click=0", "click[num]=5"
TITLE Click %click[num]% Os^^!
SET "num[list]=2 12 8 10 15 30 9 3 20 12 23 21 5"
FOR %%Q in (%num[list]%) DO (
    SET "space="
    FOR /L %%G in (1, 1, %%Q) DO (
        SET "space=!space! "
    )
    ECHO.!space!O
    SET "cod[list]=!cod[list]!%%Q.!num! "
    SET /A "num+=1"
)

CALL :RADISH_PLAY 20 10 0 GAME
...
FOR /L %%# in () DO (
    %RADISH_START_PARSE%
        IF "%%D" == "1" (
            IF not "!cod[list]:%%B.%%C=!" == "!cod[list]!" (
                ECHO Clicked^^!
                SET /A "click+=1"
                IF !click! EQU %click[num]% (
                    %RADISH_END%
                )
            )
        )
